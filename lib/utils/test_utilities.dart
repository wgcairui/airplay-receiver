...
class TestUtilities {
  ...
  static Future<void> simulateNetworkDelay({
    ...
    // 添加抖动
    final jitter =
        (baseDelay * jitterPercent * (random.nextDouble() - 0.5) * 2).round();
  ...
  static Map<String, dynamic> generateMockDeviceInfo() {
    ...
    return {
      ...
      'model': 'Test Device',
      'os_version':
          '${random.nextInt(5) + 10}.${random.nextInt(10)}.${random.nextInt(10)}',
  ...
  static Map<String, dynamic> generateMockPerformanceData({
    ...
    return {
      ...
      'memory_usage_percent':
          (baseMemoryUsage + memoryVariation).clamp(0.0, 100.0),
  ...
  static void assertInRange<T extends num>(
      T value, T min, T max, String message) {
  ...
  static Future<Map<String, dynamic>> measureResourceUsage(
    ...
    return {
      ...
      'cpu_delta': (afterMetrics['cpu_usage_percent'] ?? 0) -
          (beforeMetrics['cpu_usage_percent'] ?? 0),
      'memory_delta': (afterMetrics['memory_usage_mb'] ?? 0) -
          (beforeMetrics['memory_usage_mb'] ?? 0),
    };
  }
  ...
  static void logTestResult(String testName, bool passed,
