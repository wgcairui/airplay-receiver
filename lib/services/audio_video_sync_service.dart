...
class AudioVideoSyncService {
  ...
  void _performAdvancedSyncCorrection(double timeDifference) {
    ...
    if (totalOffset > 0) {
      ...
      _audioVideoOffset =
          _audioVideoOffset.clamp(-_maxSyncOffset, _maxSyncOffset);
  ...
  Map<String, dynamic> getSyncStats() {
    return {
      'is_in_sync': (_currentVideoTimestamp - _currentAudioTimestamp).abs() <=
          _adaptiveSyncThreshold,
      'sync_difference_ms':
          (_currentVideoTimestamp - _currentAudioTimestamp).abs(),
  ...
  void _predictiveSync() {
    ...
    // 限制预测范围，避免过度矫正
    _predictedOffset =
        _predictedOffset.clamp(-_maxSyncOffset / 2, _maxSyncOffset / 2);
  }
  ...
  void _updateAdaptiveThreshold(double currentDifference) {
    ...
    final recentJitter =
        _jitterHistory.sublist(max(0, _jitterHistory.length - 10));
    final avgJitter =
        recentJitter.reduce((a, b) => a + b) / recentJitter.length;
    ...
    _adaptiveSyncThreshold =
        _adaptiveSyncThreshold.clamp(_adaptiveThreshold, _syncThreshold * 2);
  }
  ...
  void _manageJitterBuffer(AudioVideoFrame frame, bool isVideo) {
    ...
    // 排序以确保时间戳顺序（处理乱序帧）
    final sortedList = buffer.toList()
  ...
  Map<String, dynamic> getJitterBufferStats() {
    return {
      ...
      'video_jitter_buffer_utilization':
          _jitterVideoBuffer.length / _jitterBufferSize,
      'audio_jitter_buffer_utilization':
          _jitterAudioBuffer.length / _jitterBufferSize,
