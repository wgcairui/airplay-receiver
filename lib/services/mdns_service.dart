...
class MdnsService {
  static const MethodChannel _methodChannel =
      MethodChannel('com.airplay.padcast.receiver/mdns');
  String? _localIP;
  ...
  Future<void> startAdvertising() async {
    ...
    try {
      ...
      Log.i(
          'MdnsService', 'mDNS广播服务已启动: $_localIP:${AppConstants.airplayPort}');
      ...
      if (_startAttempts < _maxRetries) {
        Log.w('MdnsService',
            '将在${_retryDelay.inSeconds}秒后重试 ($_startAttempts/$_maxRetries)');
        _scheduleRetry();
      } else {
      ...
      for (final interface in interfaces) {
        ...
        for (final addr in interface.addresses) {
          if (addr.type == InternetAddressType.IPv4 &&
            ...
            Log.d('MdnsService',
                '找到有效IP: ${addr.address} (接口: ${interface.name})');
