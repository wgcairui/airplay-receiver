...
class AirPlayService {
  ...
  Future<void> startService() async {
    ...
    try {
      ...
      // 监听设置变化
      _settingsSubscription = settingsService.settingsStream.listen(_onSettingsChanged);
  ...
  Future<void> _startHttpServer() async {
    ...
    router.get('/info', (shelf.Request request) {
      // 获取与mDNS服务一致的设备信息
      final localIP = _networkMonitor.currentNetworkInfo.ipAddress ?? '192.168.1.100';
    ...
    router.get('/server-info', (shelf.Request request) {
      final serverInfo = {
        ...
        'model': 'OPPO,Pad4Pro',
        'macAddress': _generateDeviceId(_networkMonitor.currentNetworkInfo.ipAddress ?? '192.168.1.100'),
        'features': '0x5A7FFFF7,0x1E',
        'statusFlags': '0x44',
        'protocolVersion': '1.0',
        'sourceVersion': '366.0',
  ...
  void _applyPerformanceSettings(AirPlaySettings settings) {
    ...
    switch (settings.performanceMode) {
      ...
      case PerformanceMode.performance:
        _performanceMonitor.setPerformanceProfile(PerformanceProfile.performance);
        break;
