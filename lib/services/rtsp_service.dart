...
class RtspService {
  ...
  Future<void> _startRtpReceiver() async {
    try {
      _rtpSocket =
          await RawDatagramSocket.bind(InternetAddress.anyIPv4, _rtpPort);
  ...
  void _handleRtpPacket(Uint8List data) {
    ...
    try {
      ...
      } else if (rtpHeader.payloadType == 97) {
        // AAC音频
  ...
  void _sendErrorResponse(
      RtspMessage message, int statusCode, String statusText) {
    final response = _buildRtspResponse(
      message,
      statusCode,
      statusText,
