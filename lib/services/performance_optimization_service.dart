...
class PerformanceOptimizationService {
  ...
  static const Duration _minSameTypeOptimizationInterval =
      Duration(seconds: 10);
  ...
  void _autoOptimize(PerformanceMetrics metrics) {
    ...
    if (metrics.cpuUsagePercent > _currentProfile.cpuThreshold) {
      if (_currentProfile != OptimizationProfile.batterySaver &&
          now.difference(_lastCpuOptimization) >
              _minSameTypeOptimizationInterval) {
    ...
    if (metrics.memoryUsageMB > _currentProfile.memoryThreshold) {
      if (now.difference(_lastMemoryOptimization) >
          _minSameTypeOptimizationInterval) {
    ...
    if (metrics.latencyMs > _latencyTarget * 1.5) {
      if (now.difference(_lastLatencyOptimization) >
          _minSameTypeOptimizationInterval) {
  ...
  void _adjustForHighCpuUsage() {
    ...
    // 增加缓冲区大小
    final newBufferSize =
        (_currentProfile.bufferSize * 1.2).round().clamp(3, 10);
  ...
  void _adjustForHighMemoryUsage() {
    ...
    // 减少缓冲区大小
    final newBufferSize =
        (_currentProfile.bufferSize * 0.7).round().clamp(2, 10);
