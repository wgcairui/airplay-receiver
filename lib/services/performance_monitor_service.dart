...
class PerformanceMetrics {
  ...
  double get frameDropRate => totalFrames > 0 ? (droppedFrames / totalFrames) * 100 : 0.0;
  
  String get performanceLevel {
    if (cpuUsagePercent > 80 || memoryUsagePercent > 85 || frameDropRate > 5) {
      return '差';
    } else if (cpuUsagePercent > 60 || memoryUsagePercent > 70 || frameDropRate > 2) {
      return '中等';
    } else {
      return '良好';
    }
...
class PerformanceMonitorService {
  static const MethodChannel _platformChannel = MethodChannel('com.airplay.padcast.receiver/performance');
  ...
  Future<void> startMonitoring() async {
    ...
    try {
      ...
      _resourceOptimizationTimer = Timer.periodic(const Duration(seconds: 5), (timer) {
        _optimizeResources();
      });
  ...
  void recordVideoFrame({bool dropped = false}) {
    ...
    if (elapsed.inMilliseconds >= 1000) {
      ...
      final currentDropRate = _totalFrames > 0 ? (_droppedFrames / _totalFrames) * 100 : 0.0;
  ...
  Future<void> setPerformanceProfile(PerformanceProfile profile) async {
    ...
    Log.i('PerformanceMonitorService', '切换性能配置文件: $_currentProfile -> $profile');
  ...
  void _optimizeResources() {
    ...
    try {
      ...
      if (metrics.memoryUsagePercent > _memoryAlertThreshold) {
        needsOptimization = true;
        _sendAlert('内存使用率过高: ${metrics.memoryUsagePercent.toStringAsFixed(1)}%');
      }
      
      // Check thermal state
      if (metrics.batteryTemperature > _temperatureAlertThreshold) {
        needsOptimization = true;
        _sendAlert('设备温度过高: ${metrics.batteryTemperature.toStringAsFixed(1)}°C');
      }
  ...
  Future<void> _updatePerformanceMetrics() async {
    try {
      ...
      final memoryUsagePercent = totalMemory > 0 ? (memoryUsage / totalMemory) * 100 : 0.0;
      ...
      while (_cpuHistory.length > 60) { // Keep 1 minute of history
  ...
  Future<double> _getMemoryUsage() async {
    try {
      ...
      final videoMemory = _currentFPS > 0 ? 80.0 : 20.0; // Video processing memory
  ...
  Future<Map<String, int>> _readNetworkStats() async {
    try {
      ...
      for (final line in lines) {
        if (line.contains(':') && !line.contains('lo:')) { // Skip loopback
  ...
  String getPerformanceStatus() {
    ...
    if (m.cpuUsagePercent < 30 && m.memoryUsagePercent < 50 && m.latencyMs < 40) {
      return '优秀';
    }
  ...
  String _getMemoryTrend() {
    ...
    final avgMemory = recent.map((r) => r.memoryUsage).reduce((a, b) => a + b) / recent.length;
  ...
  double _calculatePerformanceScore() {
    ...
    final frameScore = m.frameRate > 0 ? (100 - (m.frameDropRate * 10)).clamp(0, 100) : 50;
    final thermalScore = (100 - ((m.batteryTemperature - 20) * 2.5)).clamp(0, 100);
    
    // Weighted average
    const weights = [0.25, 0.25, 0.2, 0.2, 0.1]; // CPU, Memory, Latency, Frame, Thermal
    final scores = [cpuScore, memoryScore, latencyScore, frameScore, thermalScore];
  ...
  List<ResourceUsage> getResourceHistory() => List.unmodifiable(_resourceHistory);
  ...
  String _getRecommendationSeverity(PerformanceMetrics m) {
    if (m.cpuUsagePercent > 90 || m.memoryUsagePercent > 95 || m.batteryTemperature > 50) {
      return 'critical';
    }
    if (m.cpuUsagePercent > 70 || m.memoryUsagePercent > 80 || m.frameDropRate > 5) {
      return 'warning';
    }
    return 'info';
  }
