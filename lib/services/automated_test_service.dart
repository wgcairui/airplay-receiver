...
class TestResult {
  ...
  bool get isFailure => status == TestStatus.failed || status == TestStatus.timeout;
}
...
class TestReport {
  ...
  int get skippedTests => results.where((r) => r.status == TestStatus.skipped).length;
...
class AutomatedTestService {
  ...
  Future<TestReport> runTestSuite(TestSuite suite) async {
    ...
    try {
      ...
      for (final testCase in sortedTests) {
        ...
        _logTest('测试完成: ${testCase.name} - ${result.status.toString().split('.').last}');
      ...
      _logTest('测试套件完成: ${suite.name} - 成功率: ${report.successRate.toStringAsFixed(1)}%');
  ...
  Future<TestResult> _executeTestCase(TestCase testCase) async {
    ...
    try {
      ...
      Log.e('AutomatedTestService', '测试用例执行失败: ${testCase.name}', e, stackTrace);
  ...
  Map<String, dynamic> _generateSummary(List<TestResult> results) {
    ...
    return {
      ...
      'average_duration_ms': results.isNotEmpty 
        ? results.map((r) => r.duration.inMilliseconds).reduce((a, b) => a + b) / results.length
        : 0,
      'total_assertions': results.map((r) => (r.metrics['assertions'] ?? 0) as int).fold(0, (a, b) => a + b),
  ...
  double _calculateCoverage(List<TestResult> results) {
    ...
    for (final result in results) {
      if (result.isSuccess) {
        testedComponents.addAll((result.metrics['components'] as List<String>?) ?? []);
      }
    }
    ...
    return TestCase(
      ...
      execute: () async {
        ...
        try {
          ...
          metrics['assertions'] = 2;
          metrics['components'] = ['performance_monitor', 'sync_service', 'video_decoder', 'audio_decoder'];
    ...
    return TestCase(
      ...
      execute: () async {
        ...
        try {
          ...
          metrics['startup_time_ms'] = DateTime.now().difference(startTime).inMilliseconds;
      ...
      execute: () async {
        ...
        try {
          ...
          if (performanceMetrics.memoryUsagePercent > 70) {
            throw Exception('内存使用率过高: ${performanceMetrics.memoryUsagePercent}%');
          }
          ...
          metrics['memory_usage_percent'] = performanceMetrics.memoryUsagePercent;
      ...
      execute: () async {
        ...
        try {
          ...
          if (currentMetrics.memoryUsagePercent > 85) {
            throw Exception('内存使用率过高: ${currentMetrics.memoryUsagePercent.toStringAsFixed(1)}%');
          }
    ...
    return TestCase(
      ...
      execute: () async {
        ...
        try {
          ...
          final avgLatency = latencySamples.reduce((a, b) => a + b) / latencySamples.length;
    ...
    return TestCase(
      ...
      execute: () async {
        ...
        try {
          ...
          metrics['network_rx_mbps'] = currentMetrics.networkRxBytesPerSec / 1024 / 1024;
          metrics['network_tx_kbps'] = currentMetrics.networkTxBytesPerSec / 1024;
          ...
          if (rxMbps > 0 && rxMbps < 0.1) { // 如果有数据传输但过低
    ...
    return TestCase(
      ...
      execute: () async {
        ...
        try {
          ...
          final monitoring = Timer.periodic(const Duration(seconds: 5), (timer) async {
            final currentMetrics = _performanceMonitor.currentMetrics;
            if (currentMetrics != null) {
              performanceData.add({
          ...
          final avgCpu = performanceData.map((d) => d['cpu_percent'] as double).reduce((a, b) => a + b) / performanceData.length;
          final maxCpu = performanceData.map((d) => d['cpu_percent'] as double).reduce(max);
          final avgMemory = performanceData.map((d) => d['memory_percent'] as double).reduce((a, b) => a + b) / performanceData.length;
          final maxMemory = performanceData.map((d) => d['memory_percent'] as double).reduce(max);
          ...
          metrics['assertions'] = 2;
          metrics['components'] = ['performance_monitor', 'video_decoder', 'audio_decoder'];
          ...
          if (maxMemory > 95) {
            throw Exception('压力测试中内存使用率达到${maxMemory.toStringAsFixed(1)}%，系统过载');
          }
  ...
  Future<void> _simulateVideoProcessing() async {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      // 模拟一些计算密集型操作
      final _ = List.generate(1000, (index) => index * index).fold(0, (a, b) => a + b);
    }
  }
  ...
  Future<void> _simulateAudioProcessing() async {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      // 模拟音频数据处理
      final _ = List.generate(500, (index) => sin(index.toDouble())).fold(0.0, (a, b) => a + b);
    }
  }
    ...
    return TestCase(
      ...
      execute: () async {
        ...
        try {
          ...
          final syncDifference = syncStats['sync_difference_ms'] as double? ?? 0.0;
          
          if (!isInSync && syncDifference > 100) {
            throw Exception('音视频同步失败，偏差: ${syncDifference.toStringAsFixed(1)}ms');
          }
