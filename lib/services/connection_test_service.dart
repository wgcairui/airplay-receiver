...
class ConnectionTestService {
  ...
  Future<void> _testHttpService() async {
    ...
    try {
      ...
      final request = await client.get('127.0.0.1', 7000, '/test')
      ...
      if (response.statusCode == 200) {
        ...
        _updateTest(2, TestStatus.passed, 
          details: 'HTTP服务 (端口7000) 正常 - 响应: ${responseBody.substring(0, min(20, responseBody.length))}',
          duration: DateTime.now().difference(startTime));
      } else {
      ...
      if (e.toString().contains('Address already in use')) {
        ...
        errorDetails = 'HTTP服务测试失败: ${e.toString().substring(0, min(50, e.toString().length))}...';
      }
  ...
  Future<void> _testMdnsService() async {
    ...
    try {
      ...
      _updateTest(3, TestStatus.failed, 
        details: 'mDNS测试失败: ${e.toString().substring(0, min(30, e.toString().length))}...',
        duration: DateTime.now().difference(startTime));
    }
  }
  ...
  Future<void> _testRtspService() async {
    ...
    try {
      ...
      if (_airplayController != null) {
        ...
        final actualServiceRunning = _airplayController!.airplayService.isRunning;
        
        Log.d('ConnectionTestService', 'AirPlay服务状态检查: Controller=$isServiceRunning, Service=$actualServiceRunning');
  ...
  Future<void> _testPortAvailability() async {
    ...
    try {
      ...
      // 检查关键端口状态
      final httpPort = results.firstWhere((r) => r.startsWith('7000'), orElse: () => '7000:未知');
      final rtspPort = results.firstWhere((r) => r.startsWith('7001'), orElse: () => '7001:未知');
      ...
      if (hasIssues) {
        ...
        _updateTest(7, TestStatus.passed, 
          details: '端口状态正常: HTTP($httpPort) RTSP($rtspPort) mDNS(${results.last})',
          duration: DateTime.now().difference(startTime));
      }
      ...
      _updateTest(7, TestStatus.failed, 
        details: '端口检查失败: ${e.toString().substring(0, min(50, e.toString().length))}...',
        duration: DateTime.now().difference(startTime));
    }
  }
    ...
    try {
      ...
      if (_airplayController != null && _airplayController!.isServiceRunning) {
        ...
        try {
          ...
          final infoRequest = await httpClient.get(_localIP!, 7000, '/info')
          ...
          if (infoResponse.statusCode != 200) {
            ...
            final responseBody = await infoResponse.transform(utf8.decoder).join();
            final responseData = jsonDecode(responseBody);
            
            // Check required AirPlay fields for Mac compatibility
            final requiredFields = ['name', 'model', 'features', 'deviceid', 'srcvers'];
      ...
      _updateTest(8, TestStatus.failed, 
        details: 'Mac兼容性测试失败: ${e.toString().substring(0, min(50, e.toString().length))}...',
        duration: DateTime.now().difference(startTime));
    }
  }
    ...
    try {
      ...
      try {
        ...
        for (final test in infoTests) {
          try {
            final request = await httpClient.get(_localIP!, 7000, test['endpoint']!)
                .timeout(const Duration(seconds: 2));
            final response = await request.close()
                .timeout(const Duration(seconds: 2));
            ...
            if (response.statusCode == 200) {
              ...
              // Check device name
              final deviceName = data['name'] ?? data['deviceName'] ?? 'Unknown';
              ...
              results.add('${test['description']}: HTTP ${response.statusCode}');
            }
          } catch (e) {
      ...
      _updateTest(9, TestStatus.failed, 
        details: '设备信息测试失败: ${e.toString().substring(0, min(50, e.toString().length))}...',
        duration: DateTime.now().difference(startTime));
    }
  }
  
  void _updateTest(int index, TestStatus status, {String? details, Duration? duration}) {
